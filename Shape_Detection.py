...
def detect_shapes(image):
    ...
    for contour in contours:
        ...
        if area >= MIN_SHAPE_AREA:
            ...
            shape = {
                "contour": contour.tolist(),  # Convert contour to a list
...
def identify_shapes(detected_shapes, image):
    ...
    for shape in detected_shapes:
        ...
        if num_vertices == 3:
            ...
            x, y, width, height = cv2.boundingRect(np.array(shape["contour"]))  # Convert contour to a NumPy array
...
def draw_shapes(image, identified_shapes, coordinates):
    for i, shape_info in enumerate(identified_shapes):
        ...
        cv2.drawContours(image, [np.array(contour)], -1, (0, 255, 0), 2)  # Convert contour to a NumPy array
...
def main():
    ...
    data = []  # List to store the shape data
    ...
    while True:
        ...
        # Store the shape data in a list
        data.append({
