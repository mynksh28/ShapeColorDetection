...
def main():
    ...
    try:
        with open("shape_data.json", "r") as json_file:
    ...
    while True:
        ...
        for shape_info in identified_shapes:
            ...
            if "center" in shape:
                ...
                # Check if shape_info is already in data, if not, append it
                if shape_info not in data:
                    
                    data.append({
                        "label": label,
                        "color": color_name,
                        "coordinates": {"x": relative_x, "y": relative_y}
                    })
                    
                    print(f"label: {label}, Color: {color_name}, Coordinates: ({relative_x}, {relative_y})")
